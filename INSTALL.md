...
# 5) 首次建议先预览再小批量试运行（不产生正式日报/推送）
python src/main.py --collect-preview      # 只读预览：各源能抓到多少、日期合格多少
python src/main.py --trial                # 按试运行预算评分，不满意可调整来源后再正式运行

# 6) 启动网页控制台（http://127.0.0.1:8080）
...
| `processing.max_score_articles_per_run` / `trial_max_score_articles` | 100 / 30 | 每轮评分预算（新增＋积压＋失败重试合并计算）；超额延期到下轮，不会丢失也不会伪装成失败 |
| `fetcher.collection_budget_seconds` | 300 | Step 1 采集总时长上限；超时跳过剩余来源并保留游标，下次补采 |
| `fetcher.arxiv_min_interval_seconds` | 3 | arXiv ToU 最小请求间隔；多个 arXiv 订阅自动合并为一次查询（优先走分类 RSS） |
...
| `openalex.api_key` | 可选。OpenAlex 列表查询消耗额度（10 credits/次）；无 key 时额度很小，易触发 429 |

### 环境变量（优先级高于配置文件）

`DEEPSEEK_API_KEY` / `QWEN_API_KEY`、`OPENALEX_API_KEY`、`EMAIL_PASSWORD`、`FEISHU_WEBHOOK_URL`、`ZOTERO_API_KEY`、`ZOTERO_USER_ID`、`WOS_API_KEY`、`DPD_DATA_ROOT`（数据根目录）、`DPD_API` / `DPD_TOKEN` / `DPD_TIMEOUT`（仅 MCP 服务器使用）。
