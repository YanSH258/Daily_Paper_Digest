...
const Tasks = {
  ...
  renderPipelineStats(stats) {
    const keys = ["fetched_raw", "new_eligible", "new_quarantined", "needs_date", "outside_window", "invalid_date",
      "score_queue_total", "score_attempted", "score_deferred", "score_succeeded", "score_failed", "llm_requests",
    ...
    const labels = {fetched_raw:"原始抓取", new_eligible:"新文献准入", new_quarantined:"隔离", needs_date:"待日期", outside_window:"窗口外",
      ...
      score_failed:"评分失败", llm_requests:"LLM请求", source_failed:"来源失败", source_incomplete:"来源不完整"};
    ...
    return values.length ? `<p class="small">流水线统计：${values.join(" · ")}</p><p>${completion}</p>` : "";
  },
