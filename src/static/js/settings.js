...
const Settings = {
  ...
  render(c) {
    ...
    return `
      ...
      <div class="card"><h3>OpenAlex（可选）</h3>
        ${this.fieldRow("OpenAlex API Key", `<input id="s_oa_key" type="password" value="" placeholder="${c.openalex.api_key_set ? "已配置 " + API.esc(c.openalex.api_key_masked) + "（留空保持不变）" : "列表查询消耗额度，无 key 时很小；openalex.org 永久免费申请"}" />
          <button type="button" class="secondary small-btn" onclick="Settings.oaCredits()">查询剩余额度</button>
          <span id="oaCreditsMsg" class="small"></span>`)}
        <p class="small" style="margin:8px 0 0;">保存后立即生效（采集/追踪/摘要补全共用）。未配置 key 时也能用，只是额度很小容易 429。</p>
      </div>
  ...
  async oaCredits() {
    const msg = document.getElementById("oaCreditsMsg");
    if (!msg) return;
    msg.textContent = "查询中...";
    try {
      const d = await API.get("/api/openalex/credits");
      if (d.ok === false) throw new Error(API.errorMessage(d.error));
      msg.textContent = `剩余 ${d.credits_remaining} / ${d.credits_limit} credits（每 ${Math.round((d.resets_in_seconds || 0) / 3600)} 小时重置）`;
    } catch (e) {
      msg.textContent = "查询失败：" + API.errorMessage(e);
    }
  },
  ...
  async save() {
    ...
    const oaKey = val("s_oa_key").trim();
    if (oaKey) payload["openalex.api_key"] = oaKey;
