...
def load_config(path: str = "config/config.yaml") -> dict:
    # 复用主入口的配置加载，保证环境变量覆盖行为一致
    from main import load_config as _load
    return _load(path)
...
def process_one(
    ...
    全文存入独立的 fulltext_text 字段（摘要保持原文），与主流水线约定一致。
    """
    article_copy = dict(article)
    article_copy["fulltext_text"] = text
    article_copy["evidence_level"] = "FULLTEXT"
    ...
    try:
        ...
        if result.get("success") and analysis:
            ...
            conn.execute(
                "UPDATE articles SET analysis = ?, analysis_status = 'ok', "
                "analysis_model = ?, analysis_prompt_version = ?, "
                "analysis_input_hash = ?, analyzed_at = ?, "
                "fulltext_text = ?, content_hash = ?, evidence_level = 'FULLTEXT', "
                "fetch_source = 'manual_upload', analysis_error = NULL, updated_at = ? "
                "WHERE id = ?",
                (analysis, analyzer.model, PROMPT_VERSION,
                 hashlib.sha256((text + "|" + PROMPT_VERSION).encode("utf-8")).hexdigest(),
                 datetime.now().isoformat(timespec="seconds"),
                 text[:MAX_FULLTEXT_CHARS], content_hash,
                 datetime.now().isoformat(timespec="seconds"),
                 article["id"])
            )
            conn.commit()
            ...
            logger.warning("  ✗ 解读返回空或失败: %s", result.get("error", ""))
            return False
    except Exception as e:
            ...
            try:
                ...
                if result.get("success") and analysis:
                    conn.execute(
                        "UPDATE articles SET analysis = ?, analysis_status = 'ok', "
                        "analysis_model = ?, analysis_prompt_version = ?, "
                        "analyzed_at = ?, analysis_error = NULL, updated_at = ? "
                        "WHERE id = ?",
                        (analysis, analyzer.model, PROMPT_VERSION,
                         datetime.now().isoformat(timespec="seconds"),
                         datetime.now().isoformat(timespec="seconds"),
