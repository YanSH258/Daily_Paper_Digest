...
def _settings_view(ctx: WebContext) -> dict[str, Any]:
    ...
    return {
        ...
        "openalex": {
            "api_key_set": bool(openalex.get("api_key")),
            "api_key_masked": _mask_secret(openalex.get("api_key")),
        },
        "output": {
...
def _save_settings(ctx: WebContext, data: dict[str, Any]) -> tuple[dict[str, Any], int]:
    ...
    with _SETTINGS_LOCK:
        ...
        try:
            for key, value in updates.items():
                ...
                elif key == "openalex.api_key":
                    if value:  # 留空 = 保持不变
                        ensure(["openalex"])["api_key"] = str(value).strip()
                        changed.append(key)
    ...
    try:
        from integrations import openalex as _oa
        _oa.configure(ctx.config)
    except Exception:
        pass
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        try:
            ...
            if path == "/api/openalex/credits":
                if (self.ctx.config.get("web") or {}).get("protect_read") and not self._require_token():
                    return
                oa_cfg = self.ctx.config.get("openalex", {}) or {}
                api_key = (oa_cfg.get("api_key") or "").strip()
                if not api_key:
                    self._json_response({"ok": False, "error": "未配置 openalex.api_key"}, code=400)
                    return
                try:
                    import requests as _rq
                    resp = _rq.get("https://api.openalex.org/rate-limit",
                                   params={"api_key": api_key}, timeout=15)
                    if resp.status_code != 200:
                        self._json_response({"ok": False, "error": f"OpenAlex 返回 {resp.status_code}"}, code=502)
                        return
                    rl = (resp.json() or {}).get("rate_limit", {}) or {}
                    ...
                    self._json_response({"ok": False, "error": f"查询失败: {exc}"}, code=502)
                return
