...
def test_runtime_and_dev_dependency_contract():
    ...
    runtime = open(f"{root}/requirement.txt", encoding="utf-8").read().lower()
    dev = open(f"{root}/requirements-dev.in", encoding="utf-8").read().lower()
    assert "pytest" not in runtime
    assert "mcp" not in runtime
    assert "pytest==8.3.5" in dev
    assert "-r requirement.txt" in dev
