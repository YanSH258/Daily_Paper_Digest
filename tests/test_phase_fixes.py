...
class TestSettingsSave(unittest.TestCase):
    ...
    def test_openalex_api_key_save_and_blank_keeps(self):
        tmp = tempfile.mkdtemp()
        cfg = _make_tmp_config(tmp, "openalex:\n  api_key: old-key")
        ctx = self._ctx(cfg)
        # 留空 = 保持不变
        payload, code = web_server._save_settings(ctx, {"relevance_threshold": 5})
        self.assertEqual(code, 200)
        text = open(cfg, encoding="utf-8").read()
        self.assertIn("old-key", text)
        # 新值写入且运行时配置同步
        payload, code = web_server._save_settings(ctx, {"openalex.api_key": " new-key-456 "})
        self.assertEqual(code, 200)
        self.assertIn("new-key-456", open(cfg, encoding="utf-8").read())
        self.assertEqual((ctx.config.get("openalex") or {}).get("api_key"), "new-key-456")
        view = web_server._settings_view(ctx)
        self.assertTrue(view["openalex"]["api_key_set"])
