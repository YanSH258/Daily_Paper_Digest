...
- `src/processing.py`：日期准入与评分队列策略（纯函数，无副作用）；`src/utils/backlog.py`：存量处置（只读预览 / 备份后迁移 / 可恢复）；`docs/P0_PROCESSING.md`：采集管控与预算的完整约定。
...
使用项目可用的 Python 环境，不依赖特定机器的绝对路径。生产运行时安装方式为 `python3 -m pip install -r requirements.txt`；完整开发/测试环境（Python 3.10+）使用 `python3 -m pip install -r requirements-dev.txt`。没有安装依赖时先报告环境情况，不把缺依赖归为业务失败。
