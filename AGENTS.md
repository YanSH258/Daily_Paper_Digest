...
- 相关性按最匹配的研究方向判断，各方向独立且不按排列顺序分主次；不因未涉及其他方向扣分，也不因仅提及关键词给高分。评分提示词版本用 `SCORE_PROMPT_VERSION` / `score_prompt_version` 记录，解读提示词版本独立保留。
- 原先仅按标题评分的文章，补到原文摘要且仍在当前日期窗口时，进入同一评分队列并占用同一预算；已按摘要评分和手动评分的文章不自动重评。失败保留原分数、依据和成功解读，在 `score_error` 记录重评失败，失败重试仍受单轮四分之一配额限制。
- 重复发现时，只按 DOI 或 URL 精确匹配补充缺失的摘要；不能按相似标题拼接内容。手动添加明确记为 `score_model=manual`、`score_basis=manual`，统计模型评分时单独处理；历史来源不明的评分保持原状。
- RSS 日期优先取原始发表日期；Nature 官方 RSS1/RDF 的条目 `dc:date` 可以作为缺失时的回退，记作 `rss_dc_date`。Atom 的 `updated`、修改时间、频道构建时间不能当成发表时间；不让解析器把年月或非法日期补成完整日期。再次发现同一篇日期待核验文章时，仅凭有效且仍在窗口内的来源日期恢复处理，不扫描或重写全库历史。
...
- 摘要与全文分别保存；证据等级由 FetchResult 判定并持久化，不能按文本长度猜测全文，也不能将 AI 解读当成原文。仅摘要路径保存一段完整中文译文，全文路径才生成分节深度解读；两者用 `analysis_evidence_level` 区分。
